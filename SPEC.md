...
| Version | `0.1.0` au départ, **`1.0.0`** publiée sur Modrinth le 17/09/2026 ; `1.0.1` non publiée (icône du mod dans le jar) ; **`1.1.0+26.2`** ici et **`1.1.0+26.3`** sur l'autre branche depuis l'étape 20 | ✅ |
| Versions du jeu | **26.2 sur `main`, 26.3 sur `mc/26.3`** : un jar par version du jeu, jamais un binaire unique (§8 étape 20, et `../SETUP-MC-MODDING-26.3.md` §5) | ✅ |
...
| 20 | Portage 26.3 | fait **sur la branche `mc/26.3`** ; cette branche-ci reste la 26.2 et passe en `1.1.0+26.2` | **fait** 24/09/2026 |
...
| 5 | ~~Compatibilité Minecraft 26.3, en gardant 26.2~~ — **fermée le 24/09/2026.** **Deux jars, une branche par version du jeu** : cette branche reste en 26.2, `mc/26.3` porte le portage (étape 20). Le détail du portage est dans le `SPEC.md` de cette branche-là et dans `../SETUP-MC-MODDING-26.3.md` §8. Le relevé ci-dessous date du 17/09 et n'a plus qu'une valeur d'archive. | `gradle.properties`, `fabric.mod.json`, mixins, Modrinth |
...
| Date | Décision |
| --- | --- |
| 24/09/2026 | **Étape 20 : le mod est porté en 26.3, sur sa propre branche.** **Un jar par version du jeu** — `main` reste en 26.2, `mc/26.3` porte le portage — parce que la désobfuscation ne règle que les noms, pas les changements d'API (`../SETUP-MC-MODDING-26.3.md` §5). Cette branche ne change donc que de **numéro de version**, `1.1.0+26.2`, pour que les deux fichiers publiés portent le même. Le détail des ruptures 26.3 est dans le `SPEC.md` de l'autre branche et dans §8 du mémo : features refondues, `Climate.Sampler` dans `Structure.generate`, et surtout **`ServerPlayer.swing` disparue au profit d'un paquet de coup séparé**, qui n'est pas la même chose que son renommage apparent. |
