...
machine et de Minecraft 26.2 dans `../SETUP-MC-MODDING-26.2.md`.
...
| --- | --- | --- | --- |
| Balayage à vide | `ServerboundPunchPacket` (26.3 ; `ServerboundSwingPacket` en 26.2) | `ServerPunchMixin` sur `handlePunch`, juste avant le `swing` | encore lisible — le reset vient après |
...
| Icône | **une icône à nous** : l'épée dans son socle, **16 × 16** px RGBA (et non 8 × 8 comme les icônes vanilla — choix de Jérôme le 17/09/2026, étape 18). Redessinée d'après sa maquette, voir plus bas |
...
| Fichier | Rôle |
| --- | --- |
| `registry/ModMapDecorations.java` | type `mastersword:shrine` : sprite `mastersword:shrine`, teinte du manoir, mêmes drapeaux que le manoir. Enregistré dans `onInitialize` avant les datapacks |
| `textures/map/decorations/shrine.png` | l'icône 16 × 16, produite par `tools/icon/shrine_map_icon.js` (étape 18) |
...
Le type de décoration à nous porte la teinte (celui de `red_x` n'en a pas) et
le sprite. À l'étape 17, il pointait encore sur la croix rouge vanilla ;
l'étape 18 n'a eu qu'à déposer le PNG et changer l'identifiant.

#### L'icône (étape 18)

La maquette de Jérôme (`ChatGPT Image 17 sept. 2026, 18_49_02.png`) **n'est pas
une texture** : c'est une image de 1272 × 1237 px, et le sprite étiqueté
« 8 × 8 » compte en réalité **environ 11 × 25 cases**, sur une grille
irrégulière (demi-cases dans la lame). Aucune réduction automatique n'en
sortait proprement. L'icône est donc **redessinée à la main** dans
`tools/icon/shrine_map_icon.js`, une grille de caractères avec la palette
relevée sur la maquette : pommeau violet, garde bleue au cœur doré, lame
bicolore, socle de pierre en gradins.

**16 × 16 plutôt que 8 × 8** : `MapRenderer` pose le sprite sur un quadrilatère
fixe, de −1 à 1, avec les UV du sprite. Toute texture carrée s'affiche donc à la
même taille sur la carte, et une 16 × 16 garde le socle, que la version 8 × 8
réduisait à une ligne. Une texture non carrée serait étirée.
...
`../SETUP-MC-MODDING-26.2.md`. À 1000 partout, l'ordre tombait sur l'ordre
...
| --- | --- | --- |
| Mod id | `mastersword` | ✅ |
| Nom affiché du mod | **Zelda Master Sword** depuis le 17/09/2026 (choix de Jérôme, étape 19) ; « The Master Sword » avant. L'épée garde son nom en jeu | ✅ |
| Package racine | `re.jerome.mastersword` | ✅ |
| `group` Gradle | `re.jerome` | ✅ |
| `rootProject.name` | `mastersword` | ✅ |
| Version | `0.1.0` au départ, **`1.0.0`** publiée sur Modrinth le 17/09/2026 ; `1.0.1` en cours (icône du mod dans le jar) | ✅ |
| Dépôt | https://github.com/Jacob-Ps231/the-master-sword, public | ✅ |
| Modrinth | https://modrinth.com/mod/zelda-master-sword (slug `zelda-master-sword`), compte Jacob-Ps231 | ✅ |
...
└─ mixin/                    ItemStackMixin ✔ durabilité configurable,
                             ...
                             ServerPunchMixin ✔, PlayerAttackMixin ✔ vague
...
7. ~~L'API de rendu du brouillard côté client en 26.2 et la façon propre de la
   ...
   **renvoie** le `FogData` de l'image ; un `@Inject` en `RETURN` suffit, sans
   variable locale ni ordinal. Détails dans `../SETUP-MC-MODDING-26.2.md` §4.
...
| 18 | Icône de la carte | l'épée dans son socle, 16 × 16, redessinée d'après la maquette de Jérôme (§4.5) | **fait** 17/09/2026, vue en jeu par Jérôme |
| 19 | Publication | README, bloc `contact`, version 1.0.0, projet Modrinth `zelda-master-sword` en brouillon avec sa version 1.0.0, icône du mod, galerie de trois captures, projet soumis à la modération (§7.1) | **fait** 17/09/2026, en attente de la modération Modrinth |
| 20 | Portage 26.3 | branche `mc/26.3` : features refondues, `Climate.Sampler`, paquet de coup séparé, item de carte dédié, packs de ressources en format 97 | **fait** 24/09/2026, testé en jeu par Jérôme |
...
1. Les **mixins**, qui visent des méthodes précises. Pour chacun, vérifier que la méthode visée existe toujours dans 26.3, avec la même signature et au même point d'injection :
   ...
   - `PlayerAttackMixin`, `ServerPunchMixin` (`ServerPlayerMixin` en 26.2).
...
| Date | Décision |
| --- | --- |
| 24/09/2026 | **Étape 20 : le mod tourne en 26.3, et 26.2 reste servie.** **Deux jars, une branche par version du jeu** (`main` en 26.2, `mc/26.3`), comme le tranche `../SETUP-MC-MODDING-26.3.md` §5 : la désobfuscation ne règle que les noms, pas les API. Portage fait dans un **worktree**, jamais dans l'arbre principal. **52 erreurs de compilation** sur 8 fichiers : features refondues (plus de `FeaturePlaceContext` ni de paquet `configurations`, `place` à quatre arguments), `Climate.Sampler` inséré dans `Structure.generate` et son `GenerationContext`, `BiomeSource.getNoiseBiome` remplacé par un `BiomeResolver`, `RandomState.sampler()` devenu `createClimateSampler`, `MapDecorationType` tombé à trois composants, codec de bloc supprimé, `ContextAwarePredicate` devenu `Holder<LootItemCondition>`, `mulPose` devenu `rotateDegrees`. **`AbstractHugeMushroomFeature` est désormais une interface** : le mixin doit être une interface lui aussi — viser les deux records concrets ne marche pas, ils ne déclarent pas `place`. **La rupture qui comptait n'était pas dans le compilateur.** `ServerPlayer.swing(InteractionHand)` a disparu ; la re-cibler sur `swingAndResetAttackStrength` compile, s'applique, et rate quand même la moitié des cas, car 26.3 sépare les chemins : un clic gauche passe par `ServerboundPunchPacket` et `handlePunch`. Symptôme exact rapporté par Jérôme — « la vague part sur un mob, pas dans le vide ». D'où `ServerPunchMixin`. **Injections vérifiées au bytecode fusionné** (`-Dmixin.debug.export=true`), pas supposées, y compris l'ordre handler → `swing` → remise à zéro de la charge. **La carte du cartographe devient un item à nous**, `mastersword:shrine_map` : en 26.3 chaque carte d'exploration est un item dédié et `exploration_map` écrit les données **sur place** au lieu de transmuter une carte vierge, donc le nom vient de l'item et non plus d'un `set_name`. Livrée d'abord en `filled_map` sans teinte (la teinte n'existe plus en 26.3), puis Jérôme a demandé le logo : c'est ce qui a fait basculer vers l'item dédié. Texture produite par `tools/icon/shrine_map_item.js` — papier et cadre repris de la carte du manoir, pictogramme redessiné à la main dans la fenêtre 8 × 9 que vanilla réserve. Format des échanges changé au passage : `given_item_modifier` au singulier, `"type"` au lieu de `"function"`, `destination` préfixé `#`. **Packs de ressources** dupliqués en format 97 (88 en 26.2). **Relecture Opus** : aucun bug de code, trois prises utiles — `ServerPunchMixin` pas ajouté à l'index alors que `mastersword.mixins.json` le déclare `required`, de la doc restée en 26.2 dans `SPEC.md` et `AUDIT-API.md`, et le fait que `SamplerContext.EMPTY_UNCACHED` n'est pas un pis-aller mais **ce qui rend le scan sûr hors du thread serveur** (un contexte caché porte un tableau mutable), désormais écrit dans le code. Elle a confirmé au bytecode que le paquet de coup part pour un clic manqué **comme** pour un coup qui touche, et que la remise à zéro par le paquet d'attaque empêche la double vague. L'item de carte, ajouté après la relecture, n'a pas été relu : vérifié par le build, le chargement des données au serveur et l'essai en jeu de Jérôme. |
| 17/09/2026 | **Étape 19 : publication.** Le dépôt passe public sur GitHub (`main`, ex-`master`), puis le mod part sur Modrinth. **Le nom a été tranché contre l'avis donné** : « The Master Sword » était déjà pris sur Modrinth, j'ai proposé des noms libres et sans marque (« Mistbound Blade »…), Jérôme a choisi **« Zelda Master Sword »** en sachant qu'il se confond avec « Zelda: The Master Sword » (Nimbu) et qu'afficher la marque Nintendo expose à un refus de la modération. Seul le nom du mod change : l'épée, l'id `mastersword` et le dépôt gardent le leur. Concurrent le plus proche relevé : « Pedestal of the Master Sword » (Fabric, même socle) ; la page met donc en avant le brouillard, la Dark Forest, la vague de lumière et la carte. Faits : version **1.0.0**, bloc `contact` (Modrinth, sources, issues), `README.md` en anglais avec avertissement « fan-made, not affiliated with Nintendo » — valeurs relues dans le code de config et de régénération, la première rédaction décrivait mal la régénération. Sur Modrinth, par le navigateur intégré, Jérôme connecté lui-même, **confirmation demandée avant chaque action publique** : projet en brouillon, description, licence MIT, liens, catégories Adventure / Equipment / World Generation, version 1.0.0 (client + serveur requis, Fabric API requise). Le jar a été choisi par Jérôme : le navigateur intégré ne sait pas ouvrir un fichier local. **Déclaration « AI-generated content » activée** avec son accord : les règles de Modrinth l'exigent pour du code largement écrit par IA et des images générées. Pas de sous-agent de relecture : documentation et métadonnées ; vérifié par le build et par le `fabric.mod.json` lu dans le jar. **L'icône du mod**, générée par Jérôme d'après un prompt fourni (1254 × 1254, opaque, ni texte ni logo), est réduite à 512 × 512 par moyenne de zones puis réencodée en RGB avec filtres PNG et compression maximale : 228 Ko au lieu de 487, pixels identiques vérifiés au décodage, pour passer sous la limite de taille des icônes Modrinth. Elle est l'icône du projet Modrinth et `assets/mastersword/icon.png` dans le mod. Le jar 1.0.0 déjà envoyé n'en a pas : la version du dépôt passe donc à **1.0.1**, pour ne jamais avoir deux jars « 1.0.0 » différents ; la 1.0.1 sera publiée plus tard. **Galerie** : trois captures prises et triées par Jérôme (le sanctuaire dans la brume, en bannière ; l'épée dans son socle ; le socle vide, brume levée), titres et descriptions en anglais. **Projet soumis à la modération le 17/09/2026**, avec l'accord de Jérôme ; Modrinth annonce une relecture sous une semaine environ. Programme de rémunération Modrinth laissé activé par défaut, décision à Jérôme. |
| 17/09/2026 | **Étape 18 : la carte a son icône, l'épée dans son socle.** La maquette fournie par Jérôme n'était pas exploitable telle quelle : son sprite « 8 × 8 » fait environ 11 × 25 cases sur une grille irrégulière. L'icône est donc redessinée à la main, avec la palette relevée sur la maquette, dans `tools/icon/shrine_map_icon.js` (une grille de caractères, reproductible à l'octet près, comme `tools/structure/`). Deux versions présentées côte à côte avec les icônes du manoir et de la croix rouge ; **Jérôme a choisi la 16 × 16**, que le jeu affiche à la même taille qu'une 8 × 8 — `MapRenderer` pose tout sprite sur un quadrilatère fixe, lu dans les sources. Côté code : l'`assetId` du type passe de `minecraft:red_x` à `mastersword:shrine`, rien d'autre, aucun code client. **Pas de sous-agent de relecture** : une texture et un identifiant, hors des catégories obligatoires de `CLAUDE.md` ; vérifié à la place par l'en-tête du PNG (16 × 16, RGBA), sa présence dans le jar et un build vert. |
